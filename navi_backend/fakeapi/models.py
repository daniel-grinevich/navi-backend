...
class Option(UpdateRecordModel):
    ...
    def soft_delete(self, user_ip=None, user_agent=None):
        ...
        self.save(updated_fields=[
            'is_deleted', 'deleted_at', 'status', 'last_modified',
            'last_modified_user_agent', 'updated_at', 'version'
            ])
    ...
    def restore(self):
        ...
        self.status = self.Status.INACTIVE
        self.save(updated_fields=[
            'is_deleted', 'deleted_at', 'status', 'updated_at', 'version'
        ])
...
class Product(UpdateRecordModel):
    ...
    def clean(self):
        ...
        if self.status == self.Status.ACTIVE and not self.image:
            raise ValidationError({
                'status': _('Active product must have an image.')
            })

        if self.is_deleted and self.status == self.Status.ACTIVE:
            raise ValidationError({
                'status': _('Deleted products cannot be active.')
            })
